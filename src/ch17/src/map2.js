...
export default class Map {
    ...
    constructor($map) {
        ...
        const station$ = this.dragend$().pipe(
            this.getStation,
        ...
        station$.subscribe(({ markerInfo, buses }) => {
            if (this.isOpenInfoWindow(markerInfo.position)) {
                this.openInfoWindow(markerInfo.marker, markerInfo.position, this.render(markerInfo, buses));
            } else {
    ...
    dragend$() {
        return fromEvent(this.naverMap, 'dragend').pipe(
            map(({ coord }) => ({ lat: coord.y, lng: coord.x })),
        );
    }
    getStation(coord$) {
        return coord$.pipe(
            switchMap(({ lng, lat }) => ajax.getJSON(`/station/around/${lng}/${lat}`)),
    ...
    manageMarker(station$) {
        return station$.pipe(
            ...
            scan((prev, stations) => {
                prev.map(this.deleteMarker);
                prev = stations;
                return prev;
            }, []),
            mergeMap(makers => from(makers)),
        );
    }
    markerClick(maker$) {
        return maker$.pipe(
            mergeMap(maker => fromEvent(maker, 'click')),
            map(({ overlay }) => ({
                marker: overlay,
    ...
    buses(markerInfo$) {
        return markerInfo$.pipe(
            switchMap(markerInfo => {
                ...
                const buses$ = ajax.getJSON(`/bus/pass/station/${markerInfo.id}`).pipe(
                    pluck('busRouteList'),
                );
                return combineLatest(markerInfo$, buses$, (info, buses) => ({ markerInfo, buses }));
            }),
        );
    }
    render({ name }, buses) {
